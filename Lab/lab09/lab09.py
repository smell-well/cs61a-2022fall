...
class VendingMachine:
    ...
    """
    "*** YOUR CODE HERE ***"
    def __init__(self, product, cost) -> None:
    ...
    def vend(self):
        ...
        if self.fund < self.cost:
            return f'Please update your balance with ${self.cost - self.fund} more funds.'
...
def insert(link, value, index):
    ...
    curr = link.rest
    cnt = 1
    while curr != Link.empty and cnt < index:
        prev = curr
        curr = curr.rest
        cnt += 1
    
    if cnt != index or (curr == Link.empty):
        raise IndexError('Out of bounds!')
    prev.rest = Link(value, curr)
    ...
    def helper(t, reverse):
        ...
        if reverse:
            ...
            for i in range(0, n // 2, 2):
                # print("DEBUG:", i, t.branches[i].label, t.branches[n - 1 - i].label)
                a = t.branches[i].label
                t.branches[i].label = t.branches[n - 1 - i].label
                t.branches[n - 1 - i].label = a
            # print("DEBUG:", i, t.branches[i].label, t.branches[n - 1 - i].label)
        for branch in t.branches:
            helper(branch, not reverse)
