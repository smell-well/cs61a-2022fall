...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not temp.is_hive:
            ...
            temp = temp.entrance
            pos += 1
        if not attack_place: return None
            ...
            if temp.ant is not None:
                if temp == self.place:
                    if isinstance(temp.ant, LaserAnt) or \
                        (temp.ant.is_container and isinstance(temp.ant.ant_contained, LaserAnt)):
                        distance += 1
                        temp = temp.entrance
                        continue
                    res[temp.ant] = distance
                else:
                    if temp.ant.is_container and temp.ant.ant_contained is not None:
