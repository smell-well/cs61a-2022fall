...
def pingpong(n):
    ...
    def helper(i, direction):
        # print(f"DEBUG: i={i}, direction={direction}")
        if i > n:
            return 0
        if num_eights(i) > 0 or i % 8 == 0:
            return direction + helper(i + 1, -1 * direction)
        else:
...
def count_coins(change):
    ...
    """
    "*** YOUR CODE HERE ***"
    def dfs(change, allow_max_coin):
        if change == 0:
            return 1
        if change < allow_max_coin:
            ne = next_smaller_coin(allow_max_coin)
            if ne != None:
                return dfs(change, ne)
            return 0
        else:
            res = dfs(change - allow_max_coin, allow_max_coin)
            ne = next_smaller_coin(allow_max_coin)
            if ne != None:
                res += dfs(change, ne)
            return res

    return dfs(change, 25)
